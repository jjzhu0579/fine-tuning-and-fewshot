...
def process_func(example):
    try:
        ...
        if len(input_ids) != len(labels):  # 检查长度一致性
            raise ValueError("Mismatch in input_ids and labels length")
        ...
        # 返回一个空的或默认值，这样就可以跳过有问题的样本
        return {
            "input_ids": [tokenizer.pad_token_id],  # 返回最小的有效输入
            "attention_mask": [0],
            "labels": [-100]
        }
...
if __name__ == '__main__':
    ...
    lora_path = '/data/aim_nuist/aim_zhujj/xinjian/glm4_lora_ptuning/GLM4_qlora_bc5ch'

    tokenizer = AutoTokenizer.from_pretrained(glm4_model_path, use_fast=False,
                                              trust_remote_code=True)
    tokenizer.pad_token = tokenizer.eos_token
    tokenized_id = ds.map(process_func, remove_columns=ds.column_names)
    invalid_samples = [i for i in range(len(tokenized_id)) if len(tokenized_id[i]['input_ids']) == 0]
