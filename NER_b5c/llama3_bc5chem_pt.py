...
# 
# # 保存模型参数
# torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/llama_bc5chem_pt_model.pt")

# 导入模型参数
base_model = AutoModelForCausalLM.from_pretrained("/data/aim_nuist/aim_zhujj/llama3")
model = get_peft_model(base_model, config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/llama_bc5chem_pt_model.pt"))
model.to(device)

# 读取测试数据
with open("./testtry.txt", "r") as file:
...
# 预测标签并输出到output.txt
with open("llama_bc5chem_pt.txt", "w") as output_file:
    start_time = time.time()  # 记录开始时间
