...
# 保存模型参数
torch.save(model.state_dict(), "ul2_bc5chem_pt_model.pt")
...
model = get_peft_model(model, config)
model.load_state_dict(torch.load("ul2_bc5chem_pt_model.pt"))
...
# 预测标签并输出到output.txt
with open("ul2_pt_bc5chem.txt", "w") as output_file:
