...
with open('./train.tsv.txt', 'r') as file:
...
# 保存模型参数
torch.save(model.state_dict(), "t5_bc5chem_pt_model.pt")
...
model = get_peft_model(model, config)
model.load_state_dict(torch.load("t5_bc5chem_pt_model.pt"))
...
# 预测标签并输出到output.txt
with open("t5_pt_bc5chem.txt", "w") as output_file:
