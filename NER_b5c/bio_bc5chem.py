...
# 保存模型参数
torch.save(model.state_dict(), "bio_bc5chem_model.pt")

# 导入模型参数
model=AutoModelForMaskedLM.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT").to('cuda')
model.load_state_dict(torch.load("bio_bc5chem_model.pt"))
...
with open("bio_bc5chem_result.txt", "w") as output_file:
