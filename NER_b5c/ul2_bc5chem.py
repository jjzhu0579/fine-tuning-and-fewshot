...
# 保存模型参数
torch.save(model.state_dict(), "ul2_bc5chem_model.pt")

# 导入模型参数
model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/ul2")
model.load_state_dict(torch.load("ul2_bc5chem_model.pt"))
...
# 预测标签并输出到output.txt
with open("ul2_bc5chem_result.txt", "w") as output_file:
