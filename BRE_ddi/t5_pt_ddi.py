...
tokenizer = T5Tokenizer.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5",use_fast=False,trust_remote_code=True)
model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5",device_map="cuda:0", torch_dtype=torch.bfloat16,
...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/FLAN-T5_ddi_pt_model.pt")

# 导入模型参数
model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5", device_map="auto", torch_dtype=torch.bfloat16,
                                                 trust_remote_code=True).eval()
model = get_peft_model(model, config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/FLAN-T5_ddi_pt_model.pt"))
...
# 读取测试数据
with open("./combined_test.txt", "r") as file:
...
# 预测标签并输出到output.txt
with open("FLAN-T5_pt_ddi.txt", "w") as output_file:
