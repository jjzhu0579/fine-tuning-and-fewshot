...
def read_data_from_txt(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            text = " ".join(parts[1:-1])
            label = parts[-1]
            sentences.append(f"extract relationship: {text} and Determine whether there is a relationship between entities and answer true or false?")
            labels.append(label)
...
base_model = AutoModelForCausalLM.from_pretrained('/data/aim_nuist/aim_zhujj/xinjian/glm4_lora/ZhipuAI/glm-4-9b-chat',
...
with open("output_glm_qlora.txt", "w") as output_file:
    for i in range(len(test_sentence_ids)):
        sentence_id = test_sentence_ids[i]
        ...
        try:
            ...
            print(f"Error processing loop {i + 1}: {e}")
            continue
# 计算平均 F1 分数
...
with open("glm_qlora.txt", "w") as f1_output_file:
