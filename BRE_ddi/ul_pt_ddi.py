...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/ul2/ul2_ddi_pt_model.pt")
...
model = get_peft_model(model, config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/ul2/ul2_ddi_pt_model.pt"))
...
# 预测标签并输出到output.txt
with open("ul2_pt_ddi.txt", "w") as output_file:
