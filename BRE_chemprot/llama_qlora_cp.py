...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/llama_cp_qlora_model.pt")

# 导入模型参数
base_model = AutoModelForCausalLM.from_pretrained("/data/aim_nuist/aim_zhujj/llama3")
model = get_peft_model(base_model, lora_config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/llama_cp_qlora_model.pt"))
model.to(device)
...
# 预测标签并实时输出到 output.txt
with open("llama_cp_qlora.txt", "w") as output_file:
    start_time = time.time()  # 记录开始时间
    ...
    with tqdm(test_data, desc="Testing", unit="line") as ttest:
        for i, line in enumerate(ttest):
            if line.strip():  # 跳过空行
                # 进行单词或标点的标签预测
                input_text = '''please select labels from the following range according to the text : true,false ''' + line.strip()
