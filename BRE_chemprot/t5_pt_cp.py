...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/tpt_cp_model.pt")

# 导入模型参数
base_model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5")
model = get_peft_model(base_model, config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/tpt_cp_model.pt"))
...
# 预测标签并输出到output.txt
with open("tpt_cpresult.txt", "w") as output_file:
