...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/PubMedBERT/pub_cp_model.pt")

# 导入模型参数
model=AutoModelForMaskedLM.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/PubMedBERT").to('cuda')
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/PubMedBERT/pub_cp_model.pt"))
...
# Predict labels and write to output.txt
with open("pub_cpresult.txt", "w") as output_file:
