...
# 定义优化器
optimizer = torch.optim.AdamW(model.parameters(), lr=1e-5)  # 调整学习率
...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/t_pt_model.pt")

# 加载模型参数用于推理
base_model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5")
lora_model = get_peft_model(base_model, config)
lora_model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/t_pt_model.pt"))
...
with open("t_pt_hoc.txt", "w") as output_file:
