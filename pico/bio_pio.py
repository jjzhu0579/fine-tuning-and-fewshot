...
# 初始化 tokenizer 和模型
model_name = '/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT'
...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT/pub_model.pt")

model_name = '/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT'
model = AutoModelForMaskedLM.from_pretrained(model_name).to('cuda')
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT/pub_model.pt"))
...
# 预测标签并输出到output.txt
with open("output_bio11.txt", "w") as output_file:
