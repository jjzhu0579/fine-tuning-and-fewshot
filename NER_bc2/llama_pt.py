...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/llama_ner_pt_model.pt")
...
base_model = AutoModelForCausalLM.from_pretrained("/data/aim_nuist/aim_zhujj/llama3")
model = get_peft_model(base_model, config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/llama_ner_pt_model.pt"))
...
# 预测标签并输出到output.txt
with open("llama_pt.txt", "w") as output_file:
