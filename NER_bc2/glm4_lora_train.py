...
def process_func(example):
    ...
    instruction = tokenizer((f"[gMASK]<sop>\n{prompt}\n"
                             f"{example['input']}\n"
                             ).strip(),
    ...
    # 添加检查语句
    if 'input_ids' not in instruction or instruction['input_ids'] is None:
        print(f"Error: instruction['input_ids'] is None for example {example}")
    if 'input_ids' not in response or response['input_ids'] is None:
        print(f"Error: response['input_ids'] is None for example {example}")
...
if __name__ == '__main__':
    ...
    output_json = "/data/aim_nuist/aim_zhujj/xinjian/glm4_lora_ptuning/dataset_train.json"
    output_json_objects = []
    # 处理数据，遍历数据集当中的每一条记录
    ...
    lora_path = '/data/aim_nuist/aim_zhujj/xinjian/glm4_lora_ptuning/GLM4_lora'
    ...
    args = TrainingArguments(
        output_dir="/data/aim_nuist/aim_zhujj/xinjian/glm4_lora/output/GLM4",
