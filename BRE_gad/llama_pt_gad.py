...
# 构建微调文本
train_texts = [f"Based on the text: '{text}', Please determine whether there is a biomedical relationship between @GENE$ and @DISEASE$ in the text. If so, answer true, otherwise answer false" for text in train_texts]
...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/llama_gad_pt_model.pt")

import transformers
# 导入模型参数
base_model = AutoModelForCausalLM.from_pretrained("/data/aim_nuist/aim_zhujj/llama3")
model = get_peft_model(base_model, config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/llama_gad_pt_model.pt"))
model.to(device)
...
# 打开文件以写入预测结果
with open("llama_gad_pt.txt", "w", encoding='utf-8') as output_file:
    start_time = time.time()  # 记录开始时间
    ...
    with tqdm(test_data, desc="Testing", unit="line") as ttest:
        for i, line in enumerate(ttest):
            if line.strip():  # 跳过空行
                # 构建输入文本
                input_text = f"{line.strip()}"

                # 使用模型生成提示
                prompt = f"Based on the text: '{input_text}',Please determine whether there is a biomedical relationship between @GENE$ and @DISEASE$ in the text. If so, answer true, otherwise answer false"
