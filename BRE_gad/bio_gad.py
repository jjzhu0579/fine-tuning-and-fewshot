...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT/bio_bc5chem_model.pt")

# 导入模型参数
model=AutoModelForMaskedLM.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT").to('cuda')
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT/bio_bc5chem_model.pt"))
...
with open(f"x_bio_gad_result.txt", "w") as output_file:
