...
print(train_texts)
# 构建微调任务
train_texts = ['''general iob2 tags : ''' + text for text in train_texts]
...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/ul2/ul2pt_gad_model.pt")

# 导入模型参数
base_model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/ul2")
model = get_peft_model(base_model, config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/ul2/ul2pt_gad_model.pt"))
...
with open(f"ul2pt_gad_result.txt", "w") as output_file:
    for line in test_data:
        if line.strip():  # 跳过空行
            input_text = '''general iob2 tags : ''' + line.strip()
