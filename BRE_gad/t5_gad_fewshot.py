...
if __name__ == '__main__':
    ...
    generate_and_save_results(test_list, zero_shot_prompt, 'flan-t5_gad_zero_res.txt', model, tokenizer)

    # One-shot
    one_shot_prompt = prompt_base + "\n" + few_shot_examples[0]
    generate_and_save_results(test_list, one_shot_prompt, 'flan-t5_gad_one_res.txt', model, tokenizer)

    # Two-shot
    two_shot_prompt = prompt_base + "\n" + few_shot_examples[0] + few_shot_examples[1]
    generate_and_save_results(test_list, two_shot_prompt, 'flan-t5_gad_two_res.txt', model, tokenizer)

    # Three-shot
    three_shot_prompt = prompt_base + "\n" + few_shot_examples[0] + few_shot_examples[1] + few_shot_examples[2]
    generate_and_save_results(test_list, three_shot_prompt, 'flan-t5_gad_three_res.txt', model, tokenizer)

    # Five-shot
    five_shot_prompt = prompt_base + "\n" + few_shot_examples[0] + few_shot_examples[1] + few_shot_examples[2] + \
                       few_shot_examples[3] + few_shot_examples[4]
    generate_and_save_results(test_list, five_shot_prompt, 'flan-t5_gad_five_res.txt', model, tokenizer)
