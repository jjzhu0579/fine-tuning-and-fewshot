...
# 保存模型参数
torch.save(model.state_dict(), "flant5_bc5chem_model.pt")

# 导入模型参数
model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5")
model.load_state_dict(torch.load("flant5_bc5chem_model.pt"))
...
with open(f"y_t5_bc5chem_result.txt", "w") as output_file:
