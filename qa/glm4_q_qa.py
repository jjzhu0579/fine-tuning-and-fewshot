...
# 初始化 tokenizer 和模型
tokenizer = AutoTokenizer.from_pretrained('/share/home/aim/aim_zhujj/glm4_lora/ZhipuAI/glm-4-9b-chat',
...
for epoch in range(epochs):
    ...
    for batch in train_loader:
        ...
        # 添加数值稳定性处理
        if torch.isnan(outputs.loss) or torch.isinf(outputs.loss) or (outputs.loss < 0).any():
            print(f"Detected NaN, Inf, or negative loss values in epoch {epoch + 1}. Skipping batch.")
            continue
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            # 添加数值稳定性处理
            if torch.isnan(outputs.loss) or torch.isinf(outputs.loss) or (outputs.loss < 0).any():
                print(f"Detected NaN, Inf, or negative loss values in evaluation at epoch {epoch + 1}. Skipping batch.")
                continue
