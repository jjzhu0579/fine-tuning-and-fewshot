...
base_model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5",device_map="cuda:0", torch_dtype=torch.bfloat16,
...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/FLAN-T5_ptqa_model.pt")
...
lora_model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/FLAN-T5_ptqa_model.pt"))
...
with open("pt_FLAN-T5_qa.txt", "w", encoding='utf-8') as output_file:
...
# 输出JSON格式
with open("pt_FLAN-T5_qa.json", "w", encoding='utf-8') as json_file:
