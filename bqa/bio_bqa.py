...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT/bio_hoc_model.pt")

# 加载模型参数用于推理
model = AutoModelForMaskedLM.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT").to('cuda')
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/BioClinicalBERT/bio_hoc_model.pt"))
model.to(device)
