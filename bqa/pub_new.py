...
# 遍历文件夹下的所有txt文件并读取内容
folders = [f'pqal_fold{i}' for i in range(10)]
for folder in folders:
    for filename in ["train_set.txt", "dev_set.txt"]:
...
# 构建微调任务
train_texts = ['''Generate tags for each word in the given paragraph. The BIO format uses the following labels:
yes
no
...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/PubMedBERT/pub_bc5chem_model.pt")

# 导入模型参数
model = AutoModelForMaskedLM.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/PubMedBERT").to('cuda')
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/PubMedBERT/pub_bc5chem_model.pt"))
...
# 预测标签并输出到output.txt
with open("pub_qanew_result.txt", "w") as output_file:
    for line in test_data:
        if line.strip():  # Skip empty lines
            input_text = '''Generate tags for each word in the given paragraph. The BIO format uses the following labels:
yes
no
Please extract all chemicals, genes, and diseases mentioned in the paragraph. Provide the output in the format <word> - <BIO tag>, where each word is followed by its corresponding BIO tag.
