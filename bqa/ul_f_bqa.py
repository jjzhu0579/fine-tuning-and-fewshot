...
plt.legend()
plt.savefig("ul2_loss_curve.png")

# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/ul2/ul2_bqa_model.pt")

# 加载模型参数用于推理
model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/ul2")
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/ul2/ul2_bqa_model.pt"))
model.to(device)
...
results = []
with open("ul2_f_bqaresult.txt", "w", encoding='utf-8') as output_file:
