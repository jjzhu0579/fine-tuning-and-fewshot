...
plt.legend()
plt.savefig("t5pt_loss_curve.png")

# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/t5pt_bqa_model.pt")

# 加载模型参数用于推理
base_model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5")
model = get_peft_model(base_model, config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5/t5pt_bqa_model.pt"))
model.to(device)
...
results = []
with open("t_pt_bqaresult.txt", "w", encoding='utf-8') as output_file:
