...
if __name__ == '__main__':
    ...
    lora_path = '/data/aim_nuist/aim_zhujj/xinjian/glm4_lora_ptuning/GLM4_lora_bqa'

    tokenizer = AutoTokenizer.from_pretrained(glm4_model_path, use_fast=False, trust_remote_code=True)
    tokenizer.pad_token = tokenizer.eos_token

    tokenized_id = ds.map(process_func, remove_columns=["input", "output"])
    print(f"Processed dataset size: {len(tokenized_id)}")
    for i in range(5):
        print(tokenized_id[i])
    print(f"Number of training examples: {len(tokenized_id)}")
