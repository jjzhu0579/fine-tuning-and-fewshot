...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/flant5_st_model.pt")

# 导入模型参数
model = T5ForConditionalGeneration.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/FLAN-T5")
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/flant5_st_model.pt"))
...
# 预测标签并输出到output.txt
with open("t5_st_result.txt", "w") as output_file:
