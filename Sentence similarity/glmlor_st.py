...
train_texts = ['''Please understand whether the two sentences are similar or different,and only give me the answer with similar or different''' + text for text in train_texts]
...
# torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/glmlor_st_model.pt")

# Load model parameters for testing
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/glmlor_st_model.pt"))
...
with open("glmlor_st_result.txt", "w") as output_file:
    for i, line in enumerate(tqdm(test_data, desc="Testing", unit="line")):
        if line.strip():  # 跳过空行
            input_text = '''Please understand whether the two sentences are similar or different,and only give me the answer with similar or different''' + line.strip()
