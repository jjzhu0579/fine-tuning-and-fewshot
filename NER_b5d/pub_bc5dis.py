...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/pub_bc5chem_model.pt")

# 导入模型参数
model=AutoModelForMaskedLM.from_pretrained("/data/aim_nuist/aim_zhujj/xinjian/PubMedBERT").to('cuda')
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/pub_bc5chem_model.pt"))
...
# 预测标签并输出到output.txt
with open("pub_bc5disresult.txt", "w") as output_file:
