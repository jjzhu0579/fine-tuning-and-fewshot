...
# 保存模型参数
torch.save(model.state_dict(), "/data/aim_nuist/aim_zhujj/xinjian/flant5_lora_bc5dis.pt")
...
model = get_peft_model(model, config)
model.load_state_dict(torch.load("/data/aim_nuist/aim_zhujj/xinjian/flant5_lora_bc5dis.pt"))
...
# 预测标签并输出到output.txt
with open("t5_pt_bc5dis.txt", "w") as output_file:
